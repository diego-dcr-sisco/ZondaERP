...
class LotController extends Controller
{
    ...
    public function getTraceability($id){
        ...
        $orders = WarehouseOrder::with(['order', 'lot', 'user','warehouse'])
                ->where('lot_id', $id)
                ->get();
        $lot = Lot::find($id);

        $movements = WarehouseMovement::with([
            'warehouse',
            'destinationWarehouse',
            'movementType',
            'user',
            'movementProducts' => function($query) use ($id) {
                $query->where('lot_id', $id)->with(['product', 'lot']);
            }
        ])
        ->whereHas('movementProducts', function($query) use ($id) {
        ...
        return view('lot.traceability.index',compact('lot','orders','movements'));
    }
