...
@section('content')
                        ...
                        <div class="row">
                            ...
                            <div class="col-12 mb-3">
                                ...
                                <input type="text" class="form-control" id="name" name="name"
                                    value="{{ $point->name }}" placeholder="Equipo" onblur="setCode(this.value)" required>
                            </div>
                            <div class="col-12 mb-3">
                                <label for="device" class="form-label is-required">Dispositivo asociado: </label>
                                <select class="form-select" name="device_id" id="device-id" required>
                                    @foreach ($devices as $device)
                                        <option value="{{ $device->id }}"
                                            {{ $point->device_id == $device->id ? 'selected' : '' }}>{{ $device->name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-6 mb-3">
                                ...
                                <input type="color" style="height: 40px;" class="form-control-file form-control"
                                    value="{{ $point->color }}" id="color" name="color" required>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        const assign_questions = @json($assign_questions);
        ...
        $(function() {
            if (assign_questions.length > 0) {
                questions = assign_questions;
                select_questions = assign_questions.map(q => q.id)
            }
            paintQuestions();
        });
